...
// ----------------------
  ...
  //   const phoneError = document.querySelector('.message');
  //   if (phoneError) {
  //     phoneError.textContent = "message need to be less than 100 charcters.";
  //     phoneError.style.display = "block";
  //   }
  //   isValid = false;
  // }

  if(dataObj.message.length>100){
  const phoneError = document.querySelector('.message');
    if (phoneError) {
      phoneError.textContent = "message need to be less than 100 charcters.";
      phoneError.style.display = "block";
...
function fillTable(data) {
  ...
  data.forEach((row, index) => {
    ...
    tr += `<td><button class="edit-btn" data-index="${index}">Edit</button></td>`;
    tr += "</tr>";
    table.innerHTML += tr;
  });
...
document.querySelectorAll('.edit-btn').forEach(btn => {
  btn.addEventListener('click', (e) => {
    ...
    const value = data[editIndex];

    for (const key in value) {
      if (key === 'isActive') continue;
      const field = form.querySelector(`[name="${key}"]`);
      if (field) field.value = value[key];
    }
